...
ros::NodeHandle nh;
std_msgs::Int16 msg;
std_msgs::UInt16 str_msg; // Objeto que publica msg

// Nodos publicadores:

ros::Publisher chatter("arduino", &str_msg);
// Un nodo publicador puede publicar msg con el objeto que los maneja, en este caso: str_msg a el topic "arduino"

// -Nodo que activa las SALIDAS.
void messageCb(const std_msgs::String &toggle_msg)
{
  Serial.print(toggle_msg.data);
  String data;
  data = toggle_msg.data;
}
...
{
  str_msg.data = cmd_msg.data;
  chatter.publish(&str_msg);
  gripper.write(cmd_msg.data);
}

// instantiate subscribers
ros::Subscriber<centauri6dof_moveit::ArmJointState> arm_sub("joint_steps", arm_cb); // subscribes to joint_steps on arm
ros::Subscriber<std_msgs::UInt16> gripper_sub("gripper_angle", gripper_cb);         // subscribes to gripper position
ros::Subscriber<std_msgs::String> sub("salidasDigitales", &messageCb);
...
void setup()
{
  ...
  gripper.attach(GPIO_NUM_34, 700, 2350);
